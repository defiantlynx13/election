...
class Utility {
    ...
    public static function commrce_get_user_office($user_id)
    {
        $default=array(
            'is_commerce_admin' => 'no',
            'user_parent_id' =>0,
        );

        if ( metadata_exists( 'user', $user_id, 'is_commerce_admin' ) and get_user_meta($user_id,'is_commerce_admin',true) )
        {
            $default['is_commerce_admin']='yes';
        }
        else
        {
            $default['user_parent_id']=get_user_meta($user_id,'user_parent_id',true);
        }
