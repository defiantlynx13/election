...
class Core {
	...
	public function __construct() {
		if ( defined( 'ELECTION_VERSION' ) ) {
			$this->version = ELECTION_VERSION;
		} else {
			$this->version = '1.0.0';
		}
		$this->plugin_name = 'ELECTION';
...
//        $this->loader->add_action( 'admin_menu', $plugin_name_sample_admin_sub_menu2, 'add_admin_sub_menu_page' );
	...
	private function define_admin_hooks() {
        ...
        $this->loader-> add_action( 'wp_ajax_nopriv_user_login_process',$plugin_admin,'user_login_process_callback');
        $this->loader-> add_action( 'wp_ajax_nopriv_send_code_to_mobile',$plugin_admin,'send_code_to_mobile_callback');
        $this->loader-> add_action( 'wp_ajax_nopriv_confirm_code',$plugin_admin,'confirm_code_callback');
        $this->loader-> add_action( 'wp_ajax_nopriv_delete_all_sended_code',$plugin_admin,'delete_all_sended_code_callback');


        $this->loader-> add_action( 'wp_ajax_get_candidate_info',$plugin_admin,'get_candidate_info_callback');
        $this->loader-> add_action( 'wp_ajax_save_my_candidate',$plugin_admin,'save_my_candidate_callback');

        $this->loader-> add_action( 'wp_ajax_get_users_vote',$plugin_admin,'get_users_vote_callback');
        $this->loader-> add_action( 'wp_ajax_get_candidate_vote',$plugin_admin,'get_candidate_vote_callback');
        $this->loader-> add_action( 'wp_ajax_get_candidate_vote_list',$plugin_admin,'get_candidate_vote_list_callback');
        $this->loader-> add_action( 'wp_ajax_get_voted_candidate_list',$plugin_admin,'get_voted_candidate_list_callback');
        $this->loader-> add_action( 'wp_ajax_get_voting_device_data',$plugin_admin,'get_voting_device_data_callback');
    }
